...
- PPF (PlayStation Patch Format) support
  ...
  - Comprehensive test suite (20 tests) covering apply, undo, errors, metadata, and validation
...
### Added
- CLI `--only` flag for operation-specific modes
  - `--only verify`: Verify checksums without applying patch
  - Validates patch integrity and source ROM checksum
  - Skips patching and file writing
  - Foundation for future modes (metadata, validate, etc.)
...
### Fixed
- EBP format detection now correctly distinguishes from plain IPS
  - can_handle() checks for JSON metadata presence
  - EBP checked before IPS in detect_format()
  - Plain IPS patches no longer misidentified as EBP
...
### Changed
- Modularized BPS checksum validation tests into separate modules
  - Split checksum_validation_tests.rs (110 lines) into 3 modules: helpers.rs, integration_tests.rs, file_integrity_tests.rs
...
- Modularized IPS apply tests into separate modules
  - Split apply_tests.rs (119 lines) into 3 modules: basic_tests.rs, rle_tests.rs, error_tests.rs
  - All 20 tests passing

## [0.2.8-patch.3] - 2025-11-05

### Changed
- Modularized BPS apply tests into separate modules
  - Split apply_tests.rs (156 lines) into 3 modules: action_tests.rs, mixed_tests.rs, error_tests.rs
  - All 28 tests passing
...
- Modularized UPS apply tests into separate modules
  - Split apply_tests.rs (193 lines) into 4 modules: basic_tests.rs, xor_tests.rs, resize_tests.rs, error_tests.rs
  - All 26 tests passing

## [0.2.8-patch.1] - 2025-11-05

### Changed
- Modularized CLI apply command into separate modules
  - Split apply.rs (120 lines) into 3 modules: input.rs, output.rs, mod.rs
